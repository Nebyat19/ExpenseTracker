...
from shared.models import  Activity, Group, Group_Membership, Friend, Bill, Settlement
...
def dashboard(request):
    ...
    not_friend_users = User.objects.filter(~Q(id=request.user.id), ~Q(id__in=my_friends)).exclude(is_superuser=True).values('id', 'username')
...
def password_reset_confirm(request, uidb64=None, token=None):
    ...
    if user is not None:
        if request.method == 'POST':
            ...
            return render(request, 'shared/confirm_password.html')
    else:
        # print('else')
        messages.error  (request,
                        'The reset password link is no longer valid. Try again!'
...
#         }
#     return render(request, 'shared/add_friend.html', context)
...
#         'group_invites': group_invites
#     }
#     return render(request, 'shared/add_group.html', context)
